...
class VerificationCodeService
{
    ...
    public function sendEmailCode(string $email, string $type = 'default'): array
    {
        ...
        return $this->sendCodeInternal($email, true, null, $type, function ($code) use ($email, $type) {
     ...
     * @param  string  $type  验证码类型
     */
    public function verifyCode(?string $phone, ?string $areaCode, ?string $email, string $code, string $type = 'default'): bool
    {
        // 必须提供手机号或邮箱其中一个
        if (empty($phone) && empty($email)) {
            return false;
     ...
     * 发送邮件
     */
    protected function sendEmail(string $email, string $code, string $type = 'default'): void
    {
